...
////////////////////////////
#define BUFNAME 1024
void fnamepr(string action, string fname, int num)
{
	DWORD  retval = 0;
	BOOL   success;
	TCHAR  buffer[BUFNAME] = TEXT("");
	TCHAR  buf[BUFNAME] = TEXT("");
	TCHAR** lppPart = { NULL };
	if (GetFullPathName(fname.c_str(), BUFNAME, buffer, lppPart) != 0)
		fprintf(stderr, " %s %s\n", action.c_str(), buffer);
	else
...
void snapshot(bool type,int rows, int cols, string path)		
{ 
	unsigned char *pixels0;
	//pixels0 = new unsigned char[1024*1024*3];
	pixels0 = new unsigned char[cols * rows * 3];
	unsigned char *pixels;
	//pixels = new unsigned char[1024*1024*3];
	pixels = new unsigned char[cols * rows * 3];
	...
	string  appdata = path;
	if (type)
		dirname = appdata + "movie";
	else
		dirname = appdata + "screenshots";
	DWORD ftyp = GetFileAttributesA(dirname.c_str());
	if (ftyp != FILE_ATTRIBUTE_DIRECTORY)
	{
		fprintf(stderr, "Creating savedirectory % s\n", dirname.c_str());
	...
	do
	{
		num++;
		if (type)
			fname = dirname + "\\frame" + to_string(num) + ".jpg";
		else
			fname = dirname + "\\image" + to_string(num) + ".jpg";
	...
	if (err != 0)
	{
		fnamepr("Error opening ", fname.c_str(), 0);
		return;
	}
	fnamepr("Saving image ", fname.c_str(), 0);
	int quality = 90;
