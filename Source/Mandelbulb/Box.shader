...
void main()
{
	...
	if(tN < tF)
		while (mbdist < tF) 
		{
			...
			vret		= v0;
			lv0			= length(v0);
			for (iteration = 0; (iteration < maxiter) && (length(v0) < 12); iteration++) 
			{	
				if (!julia == true)	v0 = mbox(v0) * sc + vret;
				else		v0 = mbox(v0) * sc + juliav;
				lv0 = length(v0);
			}
			if (iteration >= maxiter) 
			{
				...
				for (int i = 0; i < 32; i++)
				{
					...
					vret = v0;
					lv0 = length(v0);
					for (iteration = 0; (iteration < maxiter) && (lv0 < 12); iteration++)
					{
						if (!julia == true)	v0 = mbox(v0) * sc + vret;
						else		v0 = mbox(v0) * sc + juliav;
						lv0 = length(v0);
					...
					if (iteration >= maxiter)	mbdist -= accuracy * 2.5f * binary;
					else						mbdist += accuracy * 2.5f * binary;
				}
				// find occlusion  ////////////////////////////
				if (occrange > 0)
				{	
