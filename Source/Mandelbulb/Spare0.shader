		...
		while ((l0 < tF))
		{
			...
			if (de < mindist)
			{
				...
				if (occrange > 0)
				{
					...
					mbdist = accuracy;
					de = 0;
					float l0 = length(eye - v0_);
					rayt = normalize(LightDir);		
					//reflect(normalize(vstart - camera), Normals);
					while ((l0 >  tN) && (l0 <  tF))
					{
						v0_ = neweye - rayt * mbdist;
						de = DE(v0_);
						if (de < mindist)
						{
							found = -1; 
