...
Cpair ComputePerPixelLights(vec3 E, vec3 N, vec4 p0)
{
	...
	float Occl	= 1.0f;
	if ((occrange > 0) && (p0.w < 0)) Occl	= 1.0f - occrange / 10.0f;
...
void main(){
	...
	{
		float distance	= abs(p0.w);
		if (type == 2.0 ) distance = p0.z;
		vec4 matcolor;
		if((Otrap > 0) && (type == 8.0 ))
			matcolor	= vec4(mix(col2, col1, p0.w), 1.0f);
		else
		{
			float tempdist = -cos(sine * (distance - min_) / (max_ - min_) + sincos) * 0.5f + 0.5f;
			matcolor	= vec4(mix(col2, col1, tempdist), 1.0f);
		}
		...
		Color			= matcolor.rgb * lightRes.Diff + lightRes.Spec;
		texcoord[gy][gx]= vec4(steps * vec2(1.0f/512.0f, 1.0f/400.0f) * gl_GlobalInvocationID.xy - vec2(1, 1), p0.z,1);
		float Fogfactor	= sqrt((length(eye - p0.xyz) - cmin) / (cmax - cmin) * FogEn);
		fragcol[gy][gx] = vec4(lerp(Color, FogColor, Fogfactor), 1);
	}
	else
	{
		texcoord[gy][gx]= vec4(steps * vec2(1.0f/512.0f, 1.0f/400.0f) * gl_GlobalInvocationID.xy - vec2(1, 1), 0,1);
		fragcol[gy][gx] = vec4(FogColor, 1);
	}
