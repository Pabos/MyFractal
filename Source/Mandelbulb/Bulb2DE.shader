...
void main()
{
	...
	if (d > 0)	
	{
		...
		while ((mbdist < tF) && (found == 0))
		{
			mbdist		= mbdist + 0.5 * dedist * accuracy * 50;
			v0			= eye + rayt * mbdist;
			vret		= v0;
			float dr	= 1.0;
			lv0			= length(v0);
			if (julia)
			for (int iteration = 0; iteration < maxiter; iteration +=1) 
			{	
				if (lv0 > 2.0f) break;
				dr	= order * pow(lv0, order-1.0f) * dr + 1.0f;
				v0	= rot(v0, lv0, juliav);
				lv0	= length(v0); 
				if (lv0 < mindist) mindist = lv0;  //?
			}
			else
			for (int iteration = 0; iteration < maxiter; iteration += 1)
			{
