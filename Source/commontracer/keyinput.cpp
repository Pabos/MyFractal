...
vec3 vec4to3(vec4 v) { return vec3(v.x, v.y, v.z); }
vec4 vec34(vec3 v, float f) { return vec4(v.x, v.y, v.z, f); }
void simul()
{
//Model mod = model[m];
//mat4 modrot = model[m].rot;
	float r22, t = 0;
	vec3 vt;
	vec3 rayt = vec3 (0.12, 0, -0.9925);
	...
	modrot = mat4_cast(quat(0,0,0,1));
	invrot = mat4_cast(conjugate(quat(0, 0, 0, 1))); 
	float dr = 0;
	float olddr = 0;
	float a = 0;
	float deltat = 0.005;
	...
	vec3 v;
	v = vec3(0, 0, 4) + t * rayt;
	vt = vec4to3(modrot * (vec4(v - centre, 1)));
	r22 = (vt.x * vt.x + vt.y * vt.y) - 2 * R * sqrt(vt.x * vt.x + vt.y * vt.y) + R * R + vt.z * vt.z; //equazione toro
	olddr = r - sqrt(r22);
	...
	while (t < 6) {
		...
		if (r22 < 0)
			a = a;
		if ((sqr22 - r) < epsilon) {
			vec3 rc = normalize(vec3(vt.x, vt.y, 0)) * R;
			nortemp = (invrot * vec4(normalize(vt - rc), 1));
			a = a;
		}
		else {
			dr = r - sqr22;
			deltadr = dr - olddr;
			derdr = deltadr / deltat;
			olddr = dr;
			deltat = -0.5 * dr / derdr;
		}
		//			t = t + (sqrt(r22) - mod.mdim.y) / 10;
	}
	a = a;
}
////////////////////////////
...
void snapshot(bool type,int rows, int cols)		
{ 
	unsigned char *pixels0;
	pixels0 = new unsigned char[1024*1024*3]; 
	unsigned char *pixels;
	pixels = new unsigned char[1024*1024*3]; 
			
	glReadPixels(0,0,cols,rows, 0x1907, GL_UNSIGNED_BYTE, pixels0); //RGB
	int index = 0;
	for (int row = rows; row > 0; row--)
		for (int col = 0; col < cols*3; col++)	
		{ pixels[index] = pixels0[ (row-1) * cols *3 + col]; index++;}
	...
	{ 
		...
		std::string n2;
		if(type)
			n2 = "movie/frame" + ss.str();
		else
			n2  = "screenshots/image" + ss.str();
		std::string n3  = n2 + ".jpg";
		lpszFileName = n3.c_str();
		check = fopen(lpszFileName, "r");
		if (check != 0)
		{ 
			fclose(check);
			count++;
	...
	const unsigned char *imageData = (const unsigned char *)pixels;
...
//////std::string textin;
//////std::ifstream stream("thorus.txt", std::ios::in);
//////std::string Line = "";
//////if (stream.is_open()) {
//////	getline(stream, Line);
//////}
...
//////float conv = 1;
//////vector<vec3> vertex;
//////for (int i = 0; sz + inc < Line.size(); i++)
//////{
//////	vec3 v;
//////	v.x =(stof(Line.substr(sz), &inc)) / conv; sz = sz + inc;
//////	v.y = (stof(Line.substr(sz), &inc)) / conv; sz = sz + inc;
//////	v.z = (stof(Line.substr(sz), &inc)) / conv; sz = sz + inc;
//////	vertex.push_back(v);
//////}
...
//////{
//////	int trngl;
//////	trngl = (stoi(Line.substr(sz), &inc)); sz = sz + inc;
//////	triangles.push_back(trngl);
//////}
//////int numtrng = (triangles.size() + 1) / 9;
//////for (int i = 0; i < numtrng; i++)
//////{
//////	mesh[100 + i].v0 = vec4(vertex[triangles[i * 9]], 1);
//////	mesh[100 + i].v1 = vec4(vertex[triangles[i * 9 + 3]], 1);
//////	mesh[100 + i].v2 = vec4(vertex[triangles[i * 9 + 6]], 1);
//////}
//////glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 7, Meshbuffer);
//////glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, MESHNUM * sizeof(Mesh), &mesh);
//////float a = 0;
